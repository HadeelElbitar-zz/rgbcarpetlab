    ...
    class ImageClass
    {
        ...
        public void AddSaltPepperNoise(PictureInfo pic, double salt, double pepper)
        {
            ...
            else
            {
                ...
                {
                    int ChosenIndex = rand.Next(0, Size);
                    while (!HT.ContainsKey(ChosenIndex))
                        ChosenIndex = rand.Next(0, Size);
                    Point p = (Point)HT[ChosenIndex];
                    HT.Remove(ChosenIndex);
                    int x = p.X;
                    int y = p.Y;
